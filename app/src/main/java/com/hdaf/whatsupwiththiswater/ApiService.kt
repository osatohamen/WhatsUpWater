...
data class CriticalHabitatResponse(
    val status: String,
    val data: CriticalHabitatData
)

data class CriticalHabitatData(
...
interface CriticalHabitatService {
    @GET("  ")
    fun getCriticalHabitat(
        @Header("   ") token: String,
        @Query("species_id") speciesId: String,
        @Query("date_range") dateRange: String
    ): Call<CriticalHabitatResponse>
}
...
object ApiServiceBuilder {
    ...
    val key = "    "
    val speciesId = "     "
    val dateRange = "2023-01-01 to 2023-12-31"
    val call = ApiServiceBuilder.criticalHabitatService.getCriticalHabitat(key, speciesId, dateRange)
    call.enqueue(object : Callback<CriticalHabitatResponse> {
        override fun onResponse(call: Call<CriticalHabitatResponse>, response: Response<CriticalHabitatResponse>) {
            if (response.isSuccessful) {
                val criticalHabitatData = response.body()

                // Handle the critical habitat data here
        ...
        override fun onFailure(call: Call<CriticalHabitatResponse>, t: Throwable) {
